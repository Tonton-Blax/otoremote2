...
<script>
  ...
  const MIN_RADIANS = Math.PI;
  const MAX_RADIANS = 0.000001;
  ...
  export let valueDisplayFunction = (v) => v/10;
  ...
  $: style = 'height:' + (responsive ? size + '%' : size - 5 + 'px') + ";margin-top : " + marginTop + ";";
  ...
  function updatePosition(offsetX, offsetY) {
      const dx = offsetX - size / 2; // ORIG : /2
      ...
      const tmp = Math.round((mappedValue - min) / step) * step + min; // A LA BASE C'ETAIT JUSTE CA
      value = tmp < 0 ? 0 : Math.round((mappedValue - min) / step) * step + min;
...
<div class="knob-control" style="{style}" bind:this={knob}>
  ...
  >
      ...
      <path
          ...
          bind:this={pathValue}
          data-dash="{length}"
          style="{JSON.stringify(dashStyle)}"
          class="knob-control__value">
      </path>
      <circle cx="{valueX}" cy={valueY} r="{RADIUS/5}"
          on:click="{onClick}"
